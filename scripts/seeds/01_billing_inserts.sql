...
SELECT
  ...
  12.0, 80, 71.0,
  jsonb_build_object('connection_br',1.0,'energy_br',30.0,'idle_br',40.0,'price_kwh',2.5)
FROM s1
...
SELECT
  ...
  8.5, 0, 31.75,
  jsonb_build_object('connection_br',2.0,'energy_br',29.75,'idle_br',0.0,'price_kwh',3.5)
FROM s2
ON CONFLICT (session_fk) DO UPDATE
  SET stopped_at   = EXCLUDED.stopped_at,
      energy_kwh   = EXCLUDED.energy_kwh,
