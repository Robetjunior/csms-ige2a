...
test('filtra por charge_box_id=CB-X', async () => {
  const r = await request(app).get('/v1/sessions').query({ charge_box_id: 'CB-X', sort: 'asc' }).expect(200);
...
test('filtra por status=active', async () => {
  const r = await request(app).get('/v1/sessions').query({ status: 'active' }).expect(200);
...
test('filtra por id_tag', async () => {
  const r = await request(app).get('/v1/sessions').query({ id_tag: 'ID-AAA' }).expect(200);
...
test('paginacao + ordenacao', async () => {
  const r1 = await request(app).get('/v1/sessions?sort=asc&limit=1&offset=0').expect(200);
  const r2 = await request(app).get('/v1/sessions?sort=asc&limit=1&offset=1').expect(200);
...
test('intervalo de datas (from)', async () => {
  ...
  const r = await request(app).get('/v1/sessions').query({ from }).expect(200);

  assert.ok(r.body.items.every((s: any) => new Date(s.started_at) >= new Date(from)));
});

test('filtra por transaction_id', async () => {
  const r = await request(app).get('/v1/sessions').query({ transaction_id: C }).expect(200);
